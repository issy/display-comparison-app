...
const ScreenInputCard: React.FC<ScreenInputCardProps> = ({
  ...
  return (
    <div
      ...
      style={{
        borderColor: `${screenColour}40`,
        backgroundColor: `${screenColour}10`,
      }}
    >
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-bold text-xl" style={{ color: screenColour }}>
          Screen {index + 1}
        </h3>
        {screenCount > 1 && (
          <button
      ...
      <div className="space-y-4">
        <div>
          <label
            ...
            htmlFor={`screens[${index}].diagonal`}
          >
            Diagonal (in)
          </label>
          <Controller
...
const ScreenVisualizer: React.FC<ScreenVisualizerProps> = ({
  calculatedScreens,
}) => {
  return (
    <div className="relative bg-gray-100 border-2 border-gray-300 overflow-hidden flex items-center justify-center min-h-[300px] rounded-xl shadow-inner">
      ...
      {calculatedScreens.length === 0 && (
        <p className="text-gray-500">
          Enter screen dimensions above to see the visual comparison.
        </p>
      )}
    </div>
  );
...
const App: React.FC = () => {
  ...
  const [calculatedScreens, setCalculatedScreens] = useState<
    CalculatedScreen[]
  >([]);
  ...
  const updateComparison = useCallback(
    (values: FormValues) => {
      const currentScreens = values.screens;
      ...
      if (hasError) {
        setErrorMsg(
          "Please ensure all screens have valid, positive diagonal, X, and Y values.",
        );
        setCalculatedScreens([]);
      } else {
        setErrorMsg("");
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-start py-8 px-4 bg-gray-50 font-sans">
      <div className="w-full max-w-5xl bg-white p-8 shadow-2xl rounded-xl border border-gray-100">
        ...
        <p className="text-gray-600 mb-8 text-center">
          Add, remove, and adjust screens to compare physical size and aspect
          ratio
        </p>
        ...
        <form onSubmit={handleSubmit(updateComparison)}>
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          <h2 className="text-2xl font-semibold text-gray-800 mb-4">
            Visual Comparison (1 inch = 10 pixels)
          </h2>
